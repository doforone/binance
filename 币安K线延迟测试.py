...
if __name__ == "__main__":
    ...
    for bb in bbb:
        ...
        t = threading.Thread(
            target=get_htmll,args=(
                "http://bi.kefabu.com:5000/bian/?symbol="+bb+"&interval="+aa+"&startTime="+str(
                    int(startTime*1000))+"&endTime="+str(int(time_start*1000))+"&limit="+str(limit),))
        ...
        if t.is_alive ():
            ...
            if (ddd[-1][0]/1000)//aaa[aa]==(time.time()-ab_time/1000)//aaa[aa]:
                ddd.pop()
                #print(f"{bb}: 正常")
            else:
                print(f"{bb}: 不正常--------")

            if (ddd[-1][0]/1000)//aaa[aa]==(time.time()-ab_time/1000)//aaa[aa]-1:
                #print(f"{bb}: 2正常")
                pass
            else:
                print(f"{bb}: 2不正常========")
