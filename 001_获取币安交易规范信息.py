...
if __name__ == "__main__":
    htmll=""
    urll=["https://api4.binance.com/api/v3/exchangeInfo"]
    
    while urll!=[]:
        try:
            htmll=get_htmll(urll=urll[-1])
            htmll=json.loads(htmll)
            with open(f'001.txt', 'w', encoding='utf-8', newline='\r\n') as f:
                f.write(json.dumps(htmll, indent=4, ensure_ascii=False)+"\r\n")
            urll.pop()  # 删除最后一个
        except:
            print("--Err--")
