            ...
            try:
                ...
                for d in ddd[:lenn-1]:
                    ...
                    if high_low!=0 and amount!=0:
                        ...
                        mmm_nnn.append(closee-openn)
                        lll.append({"openn":openn, "low":low, "high":high, "closee":closee, "amount":amount, "vol":vol})
                ...
                for dd in price_v:
                    if i<MN:  #==================
                        ...
                        if mmm_nnn[i]<-max(mmm_nnn[i-MN:i])*AB and mmm_nnn[i]<min(mmm_nnn[i-MN:i])*AB:  #==================
                            draw.line((i, 308, i, 308-int(((dd-minn)/maxx_minn)*300)), 'red')
                            if buy_price==0:
                                buy_numm+=1
                                buy_cur+=1
                                if buy_cur>buy_maxx:
                                    buy_maxx=buy_cur
                                buy_price=lll[i]["closee"]
                                sub_price=lll[i]["openn"]-lll[i]["closee"]
                                sub_percent=lll[i]["openn"]/lll[i]["closee"]
                            else:
                                if lll[i]["closee"]>buy_price+AA*sub_price:  #--------------------
                                    win+=1
                                    buy_cur-=1
                                    #sum_percent+=AA*sub_percent  #--------------------
                                    sum_percent+=(100*0.998/buy_price)*lll[i]["closee"]*0.998-100
                                    buy_price=0
                                    sub_price=0
                                    sub_percent=0
                                elif lll[i]["closee"]<buy_price-VV*sub_price:  ###--------------------
                                    fail+=1
                                    buy_cur-=1
                                    #sum_percent-=VV*sub_percent  ###--------------------
                                    sum_percent+=(100*0.998/buy_price)*lll[i]["closee"]*0.998-100
                                    buy_price=0
                                    sub_price=0
                                    sub_percent=0
                ...
                #print("币安：15m突破")
                image.save(f"img_15m突破/{b}_{a}_1.png", "png")
            except Exception as e:
                print(e)
                pass

        #print(f"win:{win}, fail:{fail}")
        #print(f"buy_numm:{buy_numm}")
        #print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())))

    print(f"buy_numm:{buy_numm}, win:{win}, fail:{fail}, AA:{AA}, VV:{VV}, AB:{AB}, MN:{MN}")
    print(sum_percent)
    print(buy_maxx)
    #print((((sum_percent-(buy_numm+win+fail)*0.001)/buy_maxx)*0.01+1)**2)
    print(((sum_percent/buy_maxx)*0.01+1)**1)
    print("--end--")
